...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      bscroll = new BScroll(document.querySelector(".goods-list"), { // 获取到最外层元素
        probeType: 3, // 0,1,2,3, 3 只要在运行就触发 scroll 事件
        click: true,  // 是否允许点击
        pullUpLoad: true // 上拉加载更多，默认 false
      })

      //  5-8 注册滚动事件 - scroll 事件 - 位置 position
      ...
      bscroll.on("pullingUp", () => {
        ...
        getCategoryGoods(currentOrder.value, currentCid.value).then(res => {
          goods[currentOrder.value].list.push(...res.goods.data) // 展开数据加进去
          goods[currentOrder.value].page += 1
        })
        // 完成上拉，等数据请求完成，要将新数据展示出来
        bscroll.finishPullUp()
        // console.log('当前类型：' + currentType.value + '，当前页：' + page)
        nextTick(() => { // 当 DOM 渲染完了执行方法
          // 只要页面有变化，重新计算高度
